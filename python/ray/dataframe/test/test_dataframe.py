...
def test_int_dataframe():
    ...
    pandas_df = pd.DataFrame({'col1': [0, 1, 2, 3],
                              ...
                              'col4': [12, 13, 14, 15],
                              'col5': [0, 0, 0, 0]})
    ray_df = from_pandas_df(pandas_df, 2)
...
def test_float_dataframe():
    ...
    ray_df = from_pandas_df(pandas_df, 3)
...
def test_all(ray_df, pd_df):
    assert ray_series_equals_pandas(ray_df.all(), pd_df.all())
    assert ray_series_equals_pandas(ray_df.all(axis=1), pd_df.all(axis=1))


@pytest.fixture
def test_any(ray_df, pd_df):
    assert ray_series_equals_pandas(ray_df.any(), pd_df.any())
    assert ray_series_equals_pandas(ray_df.any(axis=1), pd_df.any(axis=1))
...
def test_bfill(num_partitions=2):
    ...
    test_data.tsframe['A'][-5:] = np.nan
    ray_df = from_pandas_df(test_data.tsframe, num_partitions)
...
def test_bool(ray_df, pd_df):
    ...
    single_bool_ray_df = from_pandas_df(single_bool_pd_df, 1)
...
def test_count(ray_df, pd_df):
    assert ray_series_equals_pandas(ray_df.count(), pd_df.count())
    assert ray_series_equals_pandas(ray_df.count(axis=1), pd_df.count(axis=1))
...
def test_drop():
    ...
    ray_simple = from_pandas_df(simple, 2)
    ...
    nu_df = pd.DataFrame(pd.compat.lzip(range(3), range(-3, 1), list('abc')),
                         columns=['a', 'a', 'b'])
    ray_nu_df = from_pandas_df(nu_df, 3)
...
def test_equals():
    pandas_df1 = pd.DataFrame({'col1': [2.9, 3, 3, 3],
                               'col2': [2, 3, 4, 1]})
    ray_df1 = from_pandas_df(pandas_df1, 2)
    ray_df2 = from_pandas_df(pandas_df1, 3)

    assert ray_df1.equals(ray_df2)

    pandas_df2 = pd.DataFrame({'col1': [2.9, 3, 3, 3],
                               'col2': [2, 3, 5, 1]})
    ray_df3 = from_pandas_df(pandas_df2, 4)
...
def test_fillna_sanity(num_partitions=2):
    ...
    ray_df = from_pandas_df(test_data.tsframe, num_partitions).fillna(0)
    assert ray_df_equals_pandas(ray_df, zero_filled)

    padded = test_data.tsframe.fillna(method='pad')
    ray_df = from_pandas_df(test_data.tsframe,
    ...
    ray_df = from_pandas_df(test_data.mixed_frame,
    ...
    pytest.raises(ValueError, from_pandas_df(test_data.tsframe,
                                          num_partitions).fillna)
    with pytest.raises(ValueError):
        from_pandas_df(test_data.tsframe, num_partitions).fillna(
            5, method='ffill'
        )
    ...
    ray_df = from_pandas_df(mf, num_partitions).fillna(value=0)
    assert ray_df_equals_pandas(ray_df, result)

    result = mf.fillna(method='pad')
    ray_df = from_pandas_df(mf, num_partitions).fillna(method='pad')
    assert ray_df_equals_pandas(ray_df, result)
    ...
    ray_df = from_pandas_df(df, num_partitions).fillna({2: 'foo'})

    assert ray_df_equals_pandas(ray_df, result)

    ray_df = from_pandas_df(df, num_partitions)
    ...
    # ray_df = from_pandas_df(df, num_partitions).fillna(999, limit=1)
    ...
    ray_df = from_pandas_df(df, num_partitions).fillna(
        value={'Date': df['Date2']}
    )
    assert ray_df_equals_pandas(ray_df, result)
    ...
    ray_df = from_pandas_df(df, num_partitions).fillna(method='bfill')
...
def test_fillna_downcast(num_partitions=2):
    ...
    ray_df = from_pandas_df(df, num_partitions).fillna(0, downcast='infer')
    assert ray_df_equals_pandas(ray_df, result)
...
def test_fillna_inplace(num_partitions=2):
    ...
    ray_df = from_pandas_df(df, num_partitions).fillna(value={0: 0},
                                                    inplace=True)
...
def test_frame_fillna_limit(num_partitions=2):
    ...
    ray_df = from_pandas_df(df[:2].reindex(index), num_partitions).fillna(
        method='pad', limit=5
    )
    assert ray_df_equals_pandas(ray_df, expected)
...
def test_frame_pad_backfill_limit(num_partitions=2):
    ...
    ray_df = from_pandas_df(result, num_partitions)
    assert ray_df_equals_pandas(
...
def test_fillna_invalid_method(num_partitions=2):
    test_data = TestData()
    ray_df = from_pandas_df(test_data.frame, num_partitions)
...
def test_query(ray_df, pandas_df, funcs):
    for f in funcs:
        ...
        assert pandas_df_new.equals(to_pandas_df(ray_df_new))
...
def test_rename_sanity(num_partitions=2):
    ...
    ray_df = from_pandas_df(renamed2, num_partitions)
    assert ray_df_equals_pandas(
    ...
    ray_df = from_pandas_df(renamer, num_partitions)
...
def test_rename_nocopy(num_partitions=2):
    ...
    ray_df = from_pandas_df(test_data, num_partitions)
...
def test_rename_axis_inplace(num_partitions=2):
    ...
    ray_df = from_pandas_df(test_frame, num_partitions)
...
def test_reset_index(ray_df, pandas_df, inplace=False):
    if not inplace:
        assert to_pandas_df(ray_df.reset_index(inplace=inplace)).equals(
        ...
        assert to_pandas_df(ray_df_cp).equals(pd_df_cp)
...
def test_set_axis(ray_df, pandas_df, label, axis):
    assert to_pandas_df(ray_df.set_axis(label, axis, inplace=False)).equals(
...
def test_set_index(ray_df, pandas_df, keys, inplace=False):
    if not inplace:
        assert to_pandas_df(ray_df.set_index(keys)).equals(
...
def test___neg__(ray_df, pd_df):
    ...
    assert pd_df.__neg__().equals(to_pandas_df(ray_df_neg))
